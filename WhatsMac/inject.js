...
div[class="chat media-chat"] > div.chat-avatar { opacity: 0; } \
...
function activateSearchField() {
    document.querySelector('input[class~=\"input-search\"]').focus();
}

function newConversation() {
    document.querySelector('button[class~=\"icon-chat\"]').click();
    document.querySelector('input[class~=\"input-search\"]').focus();
    
    var header=document.querySelector("div[class~='drawer-title']");
...
function setActiveConversationAtIndex(index) {
    ...
    var conversationList = document.querySelector("div[class~='pane-list-body']");
    if (conversationList.scrollTop == 0) {
        document.querySelector('div:nth-child('+index+')[class="infinite-list-item"]').firstChild.click();
    } else {
